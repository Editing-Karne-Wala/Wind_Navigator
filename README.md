...
| Telemetry Topic | Samples | Data Density (total bytes) | Engineering Significance |
| :--- | :--- | :--- | :--- |
| **`vehicle_local_position`** | **116,343** | **22,337,856** | Validates the drone's high-frequency reaction to Wind_Navigator's discrete integer voxels. |
| **`vehicle_global_position`** | **116,213** | **7,786,271** | Confirms 4D spatial accuracy spanning the entire Manhattan bounding box. |
| **`vehicle_thrust_setpoint`** | **46,539** | **1,303,092** | Directly captures the "GLIDE" vs "THRUST" energy arbitrage logic in real-time. |
| **`vehicle_gps_position`** | **28,205** | **4,484,595** | High-fidelity satellite coordination bridging the Windows server and Linux client. |
| **`vehicle_rates_setpoint`** | **46,539** | **1,535,787** | Proves the drone's physical stability while transitioning between updrafts. |
| **`vehicle_land_detected`** | **939** | **18,780** | Successful mission termination logic after executing the 4D path. |

### **SITL Performance Summary**
The drone successfully transitioned from its "Home" location to the targeted Manhattan coordinates by interpreting macroscopic Wind_Navigator vectors as physical force deflections. By predicting the "Mechanical Lift" generated by OpenStreetMap skyscraper faces, the flight controller physically achieved a **Z-Axis climb of 50 meters** while minimizing motor duty cycles.
...
| `router_4d.py` | **Phase 8** Space-Time Pathfinding algorithm (Python Server architecture). |
